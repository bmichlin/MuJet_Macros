...
TH1F *den_1D_A0_LXY = new TH1F("den_1D_A0_LXY", "den_1D_A0_LXY", 800, 0, 80);
TH1F *num_1D_A0_LXY = new TH1F("num_1D_A0_LXY", "num_1D_A0_LXY", 800, 0, 80);

TH1F *den_1D_A0_LZ = new TH1F("den_1D_A0_LZ", "den_1D_A0_LZ", 10, 0, 80);
TH1F *num_1D_A0_LZ = new TH1F("num_1D_A0_LZ", "num_1D_A0_LZ", 10, 0, 80);
			...
			if(isDiMuonHLTFired == 1){ //Make sure that the event fired the HLT
				...
				if(is4GenMu8==true){
					...
					//if(fabs(genA0_Lz_rdet) <= 46.5){ //Second pixel endcap
						//if(genA0_Lxy_rdet <= 9.8){ //Third tracker barrel
							for(int n=0;n<160;n++){
...
*/
					...
					//if(fabs(genA1_Lz_rdet) <= 46.5){ //First pixel endcap
						//if(genA1_Lxy_rdet <= 9.8){ //First tracker barrel
							for(int n=0;n<160;n++){
...
void efficiency(){
	...
	for(auto v: DarkSUSY_mH_125_mGammaD_v) create_eff_pergamD2DLxyLz(v);
