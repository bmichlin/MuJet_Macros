...
void efficiency_hitrecovery(const std::vector<std::string>& dirNames){
			...
			if(is4SelMu8){
				...
				if(isVertexOK){
					...
					if(is2MuJets){
						...
						if(( (diMuonC_m1_FittedVtx_hitpix_l3inc==1||diMuonC_m2_FittedVtx_hitpix_l3inc==1)&&(diMuonF_m1_FittedVtx_hitpix_l3inc==1||diMuonF_m2_FittedVtx_hitpix_l3inc==1) )  || count_recovermuJetCANDF == true || count_recovermuJetC == true || count_recovermuJetF == true){
				...
				if(triggerFlag_std > 0 && triggerFlag_lxy == 0){
					...
					if(fabs(genA0_Lxy)>=11.0 || fabs(genA0_Lz)>=48.5 || fabs(genA1_Lxy)>=11.0 || fabs(genA1_Lz)>=48.5 ){ //We only want to perform this loop on a very pure sample of fakes.  Set to 11.0 instead of 9.8. 
						//Now match 
						//cout << "" << endl;
					...
					if(fabs(genA0_Lxy >= 9.8) && fabs(genA0_Lz < 48.5)) fakeCounter_dimuon1 = 1; //Fails only Lxy
					if(fabs(genA0_Lxy < 9.8) && fabs(genA0_Lz >= 48.5)) fakeCounter_dimuon1 = 2; //Fails only Lz
					if(fabs(genA0_Lxy >= 9.8) && fabs(genA0_Lz >= 48.5)) fakeCounter_dimuon1 = 3; //Fails both Lxy and Lz
					if(fabs(genA1_Lxy >= 9.8) && fabs(genA1_Lz < 48.5)) fakeCounter_dimuon2 = 4; //Fails only Lxy
					if(fabs(genA1_Lxy < 9.8) && fabs(genA1_Lz >= 48.5)) fakeCounter_dimuon2 = 5; //Fails only Lz
					if(fabs(genA1_Lxy >= 9.8) && fabs(genA1_Lz >= 48.5)) fakeCounter_dimuon2 = 6; //Fails both Lxy and Lz
	...
	void makePlot(){
		...
		TH2F *dummy3 = new TH2F("dummy3","",400,-0.2,110,100,0.0,1.0);
	...
	void FakeInvestigation()
	{
		...
		readTextFileWithSamples("/fdata/hepx/store/user/bmichlin/SampleListUntil20mm.txt", DarkSUSY_mH_125_mGammaD_v);
