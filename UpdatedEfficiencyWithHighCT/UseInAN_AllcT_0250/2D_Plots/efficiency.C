		...
		for(int k=0;k<nentries;k++){
			...
			if(isDiMuonHLTFired == 1){ //Make sure that the event fired the HLT
				...
				if(is4GenMu8==true){
					...
					//if(fabs(genA0_Lz_rdet) <= 46.5){ //First pixel endcap
						//if(genA0_Lxy_rdet <= 11.0){ //First tracker barrel
							for(int n=0;n<160;n++){
...
*/
					...
					//if(fabs(genA1_Lz_rdet) <= 46.5){ //First pixel endcap
						//if(genA1_Lxy_rdet <= 11.0){ //First tracker barrel
							for(int n=0;n<160;n++){
...
void efficiency(){
	...
	readTextFileWithSamples("/fdata/hepx/store/user/bmichlin/Samples0250_ONLY_highCT.txt", DarkSUSY_mH_125_mGammaD_v);
