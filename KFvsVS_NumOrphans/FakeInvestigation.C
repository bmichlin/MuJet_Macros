...
vector<double> mGammaD_KF_0250_eff;
vector<double> mGammaD_KF_0275_eff;
vector<double> mGammaD_KF_0300_eff;
vector<double> mGammaD_KF_2000_eff;
vector<double> mGammaD_KF_8500_eff;
vector<double> mGammaD_KF_0400_eff;
vector<double> mGammaD_KF_0700_eff;
vector<double> mGammaD_KF_1000_eff;
vector<double> mGammaD_KF_1500_eff;

vector<double> mGammaD_KF_0250_eff_uncert;
vector<double> mGammaD_KF_0275_eff_uncert;
vector<double> mGammaD_KF_0300_eff_uncert;
vector<double> mGammaD_KF_2000_eff_uncert;
vector<double> mGammaD_KF_8500_eff_uncert;
vector<double> mGammaD_KF_0400_eff_uncert;
vector<double> mGammaD_KF_0700_eff_uncert;
vector<double> mGammaD_KF_1000_eff_uncert;
vector<double> mGammaD_KF_1500_eff_uncert;

vector<double> mGammaD_KF_0250_cT;
vector<double> mGammaD_KF_0275_cT;
vector<double> mGammaD_KF_0300_cT;
vector<double> mGammaD_KF_2000_cT;
vector<double> mGammaD_KF_8500_cT;
vector<double> mGammaD_KF_0400_cT;
vector<double> mGammaD_KF_0700_cT;
vector<double> mGammaD_KF_1000_cT;
vector<double> mGammaD_KF_1500_cT;
...
addfilesMany(chain, dirNames, ext);
	...
	Int_t ev_is2DiMuonsFittedVtxOK_KF = 0;
	...
	Int_t ev_isDiMuonHLTFired_KF = 0;

	Int_t numOrphanEventsAll = 0;
	Int_t numOrphanEventsSideBand = 0;
									...
									if(diMuons_dz_FittedVtx <= 0.1 && diMuons_dz_FittedVtx >= -10){
										...
										brokenCounter++;
											...
											if(diMuonC_IsoTk_FittedVtx <= 2 && diMuonC_IsoTk_FittedVtx >= -10){
												...
												brokenCounter++;
												if(isDiMuonHLTFired){
												if((nRecoMu - 4) > 0)   numOrphanEventsSideBand++;
												if(is2DiMuonsMassOK_FittedVtx){

													ev_isDiMuonHLTFired++;

													if(is2DiMuonsFittedVtxOK_KF) ev_isDiMuonHLTFired_KF++;

													if((nRecoMu - 4) > 0)	numOrphanEventsAll++;
												}
											}
											}
										}
    ...
    std::cout << "Number of Events With An Orphan Muon (All Regions), {mass, cT, Num, Ratio}  : " << mass_double << ", " << cT_double << ", " << numOrphanEventsAll <<", " << numOrphanEventsAll/(ev_all*1.0) << std::endl; 
	    std::cout << "Number of Events With An Orphan Muon (SideBand Only), Ratio: " << mass_double << ", " << cT_double << ", " << numOrphanEventsSideBand << ", " << numOrphanEventsSideBand/(ev_all*1.0)<<  std::endl;

//KalmFitter + Vertex Scan
	if(mass_string == "0250"){
...
//KalmanFitter ONLY
	if(mass_string == "0250"){
		mGammaD_KF_0250_eff.push_back(ev_isDiMuonHLTFired_KF/(1.0*ev_4gmlxylzcut));
		mGammaD_KF_0250_eff_uncert.push_back(sqrt( ((ev_isDiMuonHLTFired_KF/(1.0*ev_4gmlxylzcut))*(1- (ev_isDiMuonHLTFired_KF/(1.0*ev_4gmlxylzcut)) ))/(1.0*ev_4gmlxylzcut)));
		mGammaD_KF_0250_cT.push_back(cT_double);
	}   
	if(mass_string == "8500"){
		mGammaD_KF_8500_eff.push_back(ev_isDiMuonHLTFired_KF/(1.0*ev_4gmlxylzcut));
		mGammaD_KF_8500_eff_uncert.push_back(sqrt( ((ev_isDiMuonHLTFired_KF/(1.0*ev_4gmlxylzcut))*(1- (ev_isDiMuonHLTFired_KF/(1.0*ev_4gmlxylzcut)) ))/(1.0*ev_4gmlxylzcut)));  
		mGammaD_KF_8500_cT.push_back(cT_double);
	}   
	if(mass_string == "0400"){
		mGammaD_KF_0400_eff.push_back(ev_isDiMuonHLTFired_KF/(1.0*ev_4gmlxylzcut));
		mGammaD_KF_0400_eff_uncert.push_back(sqrt( ((ev_isDiMuonHLTFired_KF/(1.0*ev_4gmlxylzcut))*(1- (ev_isDiMuonHLTFired_KF/(1.0*ev_4gmlxylzcut)) ))/(1.0*ev_4gmlxylzcut)));
		mGammaD_KF_0400_cT.push_back(cT_double);
	}
	if(mass_string == "1000"){
		mGammaD_KF_1000_eff.push_back(ev_isDiMuonHLTFired_KF/(1.0*ev_4gmlxylzcut));
		mGammaD_KF_1000_eff_uncert.push_back(sqrt( ((ev_isDiMuonHLTFired_KF/(1.0*ev_4gmlxylzcut))*(1- (ev_isDiMuonHLTFired_KF/(1.0*ev_4gmlxylzcut)) ))/(1.0*ev_4gmlxylzcut)));
		mGammaD_KF_1000_cT.push_back(cT_double);
	}   
	//}}}
}
...
//ONLY KF
	vector<double> null_KF_0250;
	for(int i=0; i < mGammaD_KF_0250_cT.size(); i++){
		null_KF_0250.push_back(0);
	}
	double* array_null_KF_0250               = &null_KF_0250[0];
	double* array_mGammaD_KF_0250_eff        = &mGammaD_KF_0250_eff[0];
	double* array_mGammaD_KF_0250_eff_uncert = &mGammaD_KF_0250_eff_uncert[0];
	double* array_mGammaD_KF_0250_cT         = &mGammaD_KF_0250_cT[0];
	int size_KF_0250 = null_KF_0250.size();

	TGraphErrors *gr_KF_eff_mD_0250 = new TGraphErrors(size_KF_0250,array_mGammaD_KF_0250_cT,array_mGammaD_KF_0250_eff,array_null_KF_0250,array_mGammaD_KF_0250_eff_uncert);


	vector<double> null_KF_8500;
	for(int i=0; i < mGammaD_KF_8500_cT.size(); i++){
		null_KF_8500.push_back(0);
	}
	double* array_null_KF_8500               = &null_KF_8500[0];
	double* array_mGammaD_KF_8500_eff        = &mGammaD_KF_8500_eff[0];
	double* array_mGammaD_KF_8500_eff_uncert = &mGammaD_KF_8500_eff_uncert[0];
	double* array_mGammaD_KF_8500_cT         = &mGammaD_KF_8500_cT[0];
	int size_KF_8500 = null_KF_8500.size();

	TGraphErrors *gr_KF_eff_mD_8500 = new TGraphErrors(size_KF_8500,array_mGammaD_KF_8500_cT,array_mGammaD_KF_8500_eff,array_null_KF_8500,array_mGammaD_KF_8500_eff_uncert);

	vector<double> null_KF_0400;
	for(int i=0; i < mGammaD_KF_0400_cT.size(); i++){
		null_KF_0400.push_back(0);
	}
	double* array_null_KF_0400               = &null_KF_0400[0];
	double* array_mGammaD_KF_0400_eff        = &mGammaD_KF_0400_eff[0];
	double* array_mGammaD_KF_0400_eff_uncert = &mGammaD_KF_0400_eff_uncert[0];
	double* array_mGammaD_KF_0400_cT         = &mGammaD_KF_0400_cT[0];
	int size_KF_0400 = null_KF_0400.size();

	TGraphErrors *gr_KF_eff_mD_0400 = new TGraphErrors(size_KF_0400,array_mGammaD_KF_0400_cT,array_mGammaD_KF_0400_eff,array_null_KF_0400,array_mGammaD_KF_0400_eff_uncert);

	vector<double> null_KF_1000;
	for(int i=0; i < mGammaD_KF_1000_cT.size(); i++){
		null_KF_1000.push_back(0);
	}
	double* array_null_KF_1000               = &null_KF_1000[0];
	double* array_mGammaD_KF_1000_eff        = &mGammaD_KF_1000_eff[0];
	double* array_mGammaD_KF_1000_eff_uncert = &mGammaD_KF_1000_eff_uncert[0];
	double* array_mGammaD_KF_1000_cT         = &mGammaD_KF_1000_cT[0];
	int size_KF_1000 = null_KF_1000.size();

	TGraphErrors *gr_KF_eff_mD_1000 = new TGraphErrors(size_KF_1000,array_mGammaD_KF_1000_cT,array_mGammaD_KF_1000_eff,array_null_KF_1000,array_mGammaD_KF_1000_eff_uncert);

	TLegend *leg = new TLegend(0.5,0.3,0.85,0.6);
	leg->SetBorderSize(0);
	leg->SetFillColor(0);
	leg->SetTextSize(0.045);
	leg->AddEntry(gr_eff_mD_0250,"m_{#gamma D}=0.250 GeV KalmanFitterVtx + VertexScan","PL");
	leg->AddEntry(gr_eff_mD_0400,"m_{#gamma D}=0.400 GeV KalmanFitterVtx + VertexScan","PL");
	leg->AddEntry(gr_eff_mD_1000,"m_{#gamma D}=1.000 GeV KalmanFitterVtx + VertexScan","PL");
	leg->AddEntry(gr_eff_mD_8500,"m_{#gamma D}=8.500 GeV KalmanFitterVtx + VertexScan","PL");

	leg->AddEntry(gr_KF_eff_mD_0250,"m_{#gamma D}=0.250 GeV KalmanFitterVtx","PL");
	leg->AddEntry(gr_KF_eff_mD_0400,"m_{#gamma D}=0.400 GeV KalmanFitterVtx","PL");
	leg->AddEntry(gr_KF_eff_mD_1000,"m_{#gamma D}=1.000 GeV KalmanFitterVtx","PL");
	leg->AddEntry(gr_KF_eff_mD_8500,"m_{#gamma D}=8.500 GeV KalmanFitterVtx","PL");


	TH2F *dummy3 = new TH2F("dummy3","",20,-0.2,5,100,0.0,1.0);
	...
	gr_KF_eff_mD_0250->SetLineWidth(1);
	gr_KF_eff_mD_0250->SetMarkerSize(5);
	gr_KF_eff_mD_0250->SetLineColor(1);
	gr_KF_eff_mD_0250->SetLineStyle(2);
	gr_KF_eff_mD_0250->SetMarkerColor(1);
	gr_KF_eff_mD_0250->SetMarkerStyle(26);
	gr_KF_eff_mD_0250->Draw("SAME PL");

	gr_KF_eff_mD_0400->SetLineWidth(1);
	gr_KF_eff_mD_0400->SetMarkerSize(5);
	gr_KF_eff_mD_0400->SetLineColor(7);
	gr_KF_eff_mD_0400->SetLineStyle(2);
	gr_KF_eff_mD_0400->SetMarkerColor(7);
	gr_KF_eff_mD_0400->SetMarkerStyle(26);
	gr_KF_eff_mD_0400->Draw("SAME PL");

	gr_KF_eff_mD_1000->SetLineWidth(1);
	gr_KF_eff_mD_1000->SetMarkerSize(5);
	gr_KF_eff_mD_1000->SetLineColor(9);
	gr_KF_eff_mD_1000->SetLineStyle(2);
	gr_KF_eff_mD_1000->SetMarkerColor(9);
	gr_KF_eff_mD_1000->SetMarkerStyle(26);
	gr_KF_eff_mD_1000->Draw("SAME PL");

	gr_KF_eff_mD_8500->SetLineWidth(1);
	gr_KF_eff_mD_8500->SetMarkerSize(5);
	gr_KF_eff_mD_8500->SetLineColor(6);
	gr_KF_eff_mD_8500->SetLineStyle(2);
	gr_KF_eff_mD_8500->SetMarkerColor(6);
	gr_KF_eff_mD_8500->SetMarkerStyle(26);
	gr_KF_eff_mD_8500->Draw("SAME PL");

	leg->Draw("same");
	c->SaveAs("e_rec_alpha_gen_vs_cT_0250_GeV.pdf","recreate");
	c->SaveAs("e_rec_alpha_gen_vs_cT_0250_GeV.C","recreate");
}
