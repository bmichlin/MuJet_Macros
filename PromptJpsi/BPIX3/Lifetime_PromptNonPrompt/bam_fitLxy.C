...
data->SetLineColor(kRed);
	...
	if (status == 0) {                       // check on fit status
		...
		leg9->AddEntry(result,"(prompt + non-prompt) J#psi J#psi","FL");
		data->SetTitle("CMS 2015  #sqrt{s} = 13 TeV  L_{int} = 2.83 fb^{-1}");
		...
		//txtHeader->SetHeader("CMS 2015  #sqrt{s} = 13 TeV  L_{int} = 2.83 fb^{-1}");
