	...
	while ((chEl=(TChainElement*)next())) {
		...
		for(int k=0;k<t->GetEntries();k++){
			...
			if((type == 1 || type == 0) || (type == 2 && jpsi_trigger == 1)){ //Turn the trigger off for MC
				...
				if(jpsi_inv_mass_dimuon1 > 3.3 || jpsi_inv_mass_dimuon1 < 2.8 || jpsi_inv_mass_dimuon2 > 3.3 || jpsi_inv_mass_dimuon2 < 2.8) continue; //Do the exact same thing for data and MC
				...
				if(jpsi_isoTk_dimuon1 < 0 || jpsi_isoTk_dimuon2 < 0 || jpsi_isoTk_dimuon1 > 12 || jpsi_isoTk_dimuon2 > 12) continue; // Make sure that we are within our [0-12] search region
...
void jpsiPlots()
{
	...
	analyze("/fdata/hepx/store/user/bmichlin/DPS_BPIX3/HighPT_Thresholds/Combined/", 0); //DPS MC //High and low may be mislabeled? //YES they are mislabeled!!!
