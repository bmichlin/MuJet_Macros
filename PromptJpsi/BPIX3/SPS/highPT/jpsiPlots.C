...
TH1F *t1_jpsi_sps_event_failure_type = new TH1F("t1_jpsi_sps_event_failure_type","t1_jpsi_sps_event_failure_type",7, 0, 7);
TH1F *t1_jpsi_sps_inv_mass_dimuon1   = new TH1F("t1_jpsi_sps_inv_mass_dimuon1","t1_jpsi_sps_inv_mass_dimuon1",75, 0, 15);
TH1F *t1_jpsi_sps_inv_mass_dimuon2   = new TH1F("t1_jpsi_sps_inv_mass_dimuon2","t1_jpsi_sps_inv_mass_dimuon2",75, 0, 15);
TH1F *t1_jpsi_sps_mass_dimuon1       = new TH1F("t1_jpsi_sps_mass_dimuon1","t1_jpsi_sps_mass_dimuon1",75, 0, 15);
TH1F *t1_jpsi_sps_mass_dimuon2       = new TH1F("t1_jpsi_sps_mass_dimuon2","t1_jpsi_sps_mass_dimuon2",75, 0, 15);
TH1F *t1_jpsi_sps_invmass_1plus2        = new TH1F("t1_jpsi_sps_invmass_1plus2","t1_jpsi_sps_invmass_1plus2",140, 0, 140);
TH1F *t1_jpsi_sps_isoTk_dimuon1      = new TH1F("t1_jpsi_sps_isoTk_dimuon1","t1_jpsi_sps_isoTk_dimuon1",48, 0, 12);
TH1F *t1_jpsi_sps_isoTk_dimuon2      = new TH1F("t1_jpsi_sps_isoTk_dimuon2","t1_jpsi_sps_isoTk_dimuon2",48, 0, 12);
TH1F *t1_jpsi_sps_deltay             = new TH1F("t1_jpsi_sps_deltay", "t1_jpsi_sps_deltay", 50, 0, 5);

TH2F *t2_jpsi_sps_inv_mass		 = new TH2F("t2_jpsi_sps_inv_mass", "t2_jpsi_sps_inv_mass", 100, 0, 5, 100, 0, 5);
TH2F *t2_jpsi_sps_iso		 = new TH2F("t2_jpsi_sps_iso", "t2_jpsi_sps_iso", 48, 0, 12, 48, 0, 12);
...
void analyze(TString fileName){
	...
	while ((chEl=(TChainElement*)next())) {
		...
		for(int k=0;k<t->GetEntries();k++){
			t->GetEntry(k);
			t1_jpsi_sps_event_failure_type->Fill(jpsi_event_failure_type);
...
//cout << "nTuple was filled.  Filling plots." << endl;
					...
					t1_jpsi_sps_inv_mass_dimuon1->Fill(jpsi_inv_mass_dimuon1);  
					t1_jpsi_sps_inv_mass_dimuon2->Fill(jpsi_inv_mass_dimuon2); 
					t1_jpsi_sps_mass_dimuon1->Fill(jpsi_mass_dimuon1);      
					t1_jpsi_sps_mass_dimuon2->Fill(jpsi_mass_dimuon2);      
					t2_jpsi_sps_inv_mass->Fill(jpsi_inv_mass_dimuon1, jpsi_inv_mass_dimuon2);
...
t1_jpsi_sps_invmass_1plus2->Fill(sqrt( ((p1+p2)*(p1+p2)) - (e1+e2)*(e1-e2) ));
...
cout << "px, py, pz, p1, m1, e, num, den, ln, y: " << jpsi_px_dimuon1 << ", " << jpsi_py_dimuon1 << ", " << jpsi_pz_dimuon1 << ", " << p1 << ", " << jpsi_inv_mass_dimuon1 << ", " << e1 << ", " << num1 << ", " << den1 << ", " << ln1 << ", " << y1 << endl;
					t1_jpsi_sps_deltay->Fill(dy);

					if(jpsi_isoTk_dimuon1 >= 0 && jpsi_isoTk_dimuon2 >= 0){

						t1_jpsi_sps_isoTk_dimuon1->Fill(jpsi_isoTk_dimuon1); 
						t1_jpsi_sps_isoTk_dimuon2->Fill(jpsi_isoTk_dimuon2);     
						t2_jpsi_sps_iso->Fill(jpsi_isoTk_dimuon1, jpsi_isoTk_dimuon2);
					}
				}
			}
...
void makePlotsPretty(){
	...
	t1_jpsi_sps_event_failure_type->Draw();
	c1->SaveAs("Event_Failure_Type.pdf");
	c1->Clear();

	t1_jpsi_sps_inv_mass_dimuon1->GetXaxis()->SetTitle("m_{#mu#mu_{1}} [GeV/c^{2}]");
	t1_jpsi_sps_inv_mass_dimuon1->GetYaxis()->SetTitle("Fraction of Events");
	t1_jpsi_sps_inv_mass_dimuon1->DrawNormalized();
	c1->SaveAs("InvMass1.pdf");
	c1->Clear();
	
	t1_jpsi_sps_inv_mass_dimuon2->GetXaxis()->SetTitle("m_{#mu#mu_{2}} [GeV/c^{2}]");
	t1_jpsi_sps_inv_mass_dimuon2->GetYaxis()->SetTitle("Fraction of Events");
	t1_jpsi_sps_inv_mass_dimuon2->DrawNormalized();
	c1->SaveAs("InvMass2.pdf");
	c1->Clear();

	t1_jpsi_sps_mass_dimuon1->GetXaxis()->SetTitle("m_{#mu#mu_{1}} [GeV/c^{2}]");
	t1_jpsi_sps_mass_dimuon1->GetYaxis()->SetTitle("Fraction of Events");
	t1_jpsi_sps_mass_dimuon1->DrawNormalized();
	c1->SaveAs("Mass1.pdf");
	c1->Clear();

	t1_jpsi_sps_mass_dimuon2->GetXaxis()->SetTitle("m_{#mu#mu_{2}} [GeV/c^{2}]");
	t1_jpsi_sps_mass_dimuon2->GetYaxis()->SetTitle("Fraction of Events");
	t1_jpsi_sps_mass_dimuon2->DrawNormalized();
	c1->SaveAs("Mass2.pdf");
	c1->Clear();

	t1_jpsi_sps_invmass_1plus2->GetXaxis()->SetTitle("m_{#mu#mu_{1} + #mu#mu_{2}} [GeV/c^{2}]");
	t1_jpsi_sps_invmass_1plus2->GetYaxis()->SetTitle("Fraction of Events");
	t1_jpsi_sps_invmass_1plus2->DrawNormalized();
	c1->SaveAs("InvMass1plus2.pdf");
	c1->Clear();
	
	t1_jpsi_sps_isoTk_dimuon1->GetXaxis()->SetTitle("Iso_{#mu#mu_{1}} [GeV/c]");
	t1_jpsi_sps_isoTk_dimuon1->GetYaxis()->SetTitle("Fraction of Events");
	t1_jpsi_sps_isoTk_dimuon1->DrawNormalized();
	c1->SaveAs("IsoTk_1.pdf");
	c1->Clear();

	t1_jpsi_sps_isoTk_dimuon2->GetXaxis()->SetTitle("Iso_{#mu#mu_{2}} [GeV/c]");
	t1_jpsi_sps_isoTk_dimuon2->GetYaxis()->SetTitle("Fraction of Events");
	t1_jpsi_sps_isoTk_dimuon2->DrawNormalized();
	c1->SaveAs("IsoTk_2.pdf");
	c1->Clear();
	
	t1_jpsi_sps_deltay->GetXaxis()->SetTitle("|#Delta y|");
	t1_jpsi_sps_deltay->GetYaxis()->SetTitle("Fraction of Events");
	t1_jpsi_sps_deltay->DrawNormalized();
	c1->SaveAs("deltaY.pdf");
	c1->Clear();
	
	t2_jpsi_sps_inv_mass->GetXaxis()->SetTitle("m_{#mu#mu_{1}} [GeV/c^{2}]");
	t2_jpsi_sps_inv_mass->GetYaxis()->SetTitle("m_{#mu#mu_{2}} [GeV/c^{2}]");
	gPad->SetLogz(1);
	t2_jpsi_sps_inv_mass->Draw("colz");
