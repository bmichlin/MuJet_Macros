	...
	while ((chEl=(TChainElement*)next())) {
		...
		for(int k=0;k<t->GetEntries();k++){
			...
			if((type == 1 || type == 0) || (type == 2 && jpsi_trigger == 1)){ //Turn the trigger off for MC
				...
				if((jpsi_isoTk_dimuon1 < 0 || jpsi_isoTk_dimuon2 < 0 || jpsi_isoTk_dimuon1 > 12 || jpsi_isoTk_dimuon2 > 12)) continue; //Same for data and MC. Only use iso 0-12?
...
void makePlotsPretty(){
	...
	t1_data_jpsi_DeltaRapidityC->Scale(6.0/t1_data_np_regionC_dimuon1_lxy->Integral()); //6.0 is number of non-prompt events from ABCD method
	...
	t1_data_jpsi_DeltaRapidityA->SetTitle("CMS 2015  #sqrt{s} = 13 TeV  L_{int} = 2.83 fb^{-1}");
	t1_data_jpsi_DeltaRapidityC->GetXaxis()->SetTitle("|#Delta y|");
	t1_data_jpsi_DeltaRapidityC->GetYaxis()->SetTitle("Events/0.5 GeV");
	t1_data_jpsi_DeltaRapidityC->SetTitle("CMS 2015  #sqrt{s} = 13 TeV  L_{int} = 2.83 fb^{-1}");
	...
	t1_sps_jpsi_DeltaRapidity->Scale(6.0/t1_sps_jpsi_DeltaRapidity->Integral());
	t1_dps_jpsi_DeltaRapidity->Scale(6.0/t1_dps_jpsi_DeltaRapidity->Integral());
...
void jpsiPlots()
{
	...
	analyze("/fdata/hepx/store/user/bmichlin/DPS_BPIX3/HighPT_Thresholds/Combined/", 0); //DPS MC //High and Low are mixed up for DPS!
	analyze("/fdata/hepx/store/user/bmichlin/SPS_BPIX3/LowPT_Threshold/", 1); //SPS MC
    analyze("/fdata/hepx/store/user/bmichlin/MuOnia/DoubleJpsi_2015C_763_patTuple_cutana_mujets_Run2015C_ReReco_AOD_V1/160715_213415/0000/", 2); //Data
	analyze("/fdata/hepx/store/user/bmichlin/MuOnia/DoubleJpsi_2015D_763_patTuple_cutana_mujets_Run2015D_ReReco_AOD_V1/160715_213318/0000/", 2); //Data
	analyze("/fdata/hepx/store/user/bmichlin/MuOnia/DoubleJpsi_2015D_763_patTuple_cutana_mujets_Run2015D_ReReco_AOD_V1/160715_213318/0001/", 2); //Data
	analyze("/fdata/hepx/store/user/bmichlin/MuOnia/DoubleJpsi_2015D_763_patTuple_cutana_mujets_Run2015D_ReReco_AOD_V1/160715_213318/0002/", 2); //Data
	analyze("/fdata/hepx/store/user/bmichlin/MuOnia/DoubleJpsi_2015D_763_patTuple_cutana_mujets_Run2015D_ReReco_AOD_V1/160715_213318/0003/", 2); //Data
	analyze("/fdata/hepx/store/user/bmichlin/MuOnia/DoubleJpsi_2015D_763_patTuple_cutana_mujets_Run2015D_ReReco_AOD_V1/160715_213318/0004/", 2); //Data
	makePlotsPretty();
	BAM->Write();
}
