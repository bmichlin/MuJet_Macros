...
//cout << "nTuple was filled.  Filling plots." << endl;
				...
				if(jpsi_isoTk_dimuon1 < 0 || jpsi_isoTk_dimuon2 < 0 || jpsi_isoTk_dimuon1 > 12 || jpsi_isoTk_dimuon2 > 12) continue;
				//Check correlation



				//Count number of entries in each region
				if(jpsi_isoTk_dimuon1 <= 2 && jpsi_isoTk_dimuon2 <= 2) regionA++;
				if(jpsi_isoTk_dimuon1 <= 2 && jpsi_isoTk_dimuon2 > 2 ) regionB++;
				if(jpsi_isoTk_dimuon1 > 2  && jpsi_isoTk_dimuon2 > 2 ) regionC++;
...
void getCorrAndStats(){
	cout << "Correlation factor between IsoTK of dimuon1 and 2: " << t2_jpsi_iso->GetCorrelationFactor() << endl;
