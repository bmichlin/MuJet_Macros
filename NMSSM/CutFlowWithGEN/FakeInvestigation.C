...
void efficiency_hitrecovery(const std::vector<std::string>& dirNames){
	...
	decodeFileNameManyNMSSM(dirNames, mass_string, cT_string);
	fileName = "NMSSM_mH_" + mass_string + "_mA_" + cT_string;
...
*/
			...
			//GEN cutflow by hand
			double genMu00_pt = sqrt(genA0Mu0_px*genA0Mu0_px + genA0Mu0_py*genA0Mu0_py);
			double genMu01_pt = sqrt(genA0Mu1_px*genA0Mu1_px + genA0Mu1_py*genA0Mu1_py);
			double genMu10_pt = sqrt(genA1Mu0_px*genA1Mu0_px + genA1Mu0_py*genA1Mu0_py);
			double genMu11_pt = sqrt(genA1Mu1_px*genA1Mu1_px + genA1Mu1_py*genA1Mu1_py);

			vector< double > pT_eta00 = {genMu00_pt, genA0Mu0_eta};
			vector< double > pT_eta01 = {genMu01_pt, genA0Mu1_eta};
			vector< double > pT_eta10 = {genMu10_pt, genA1Mu0_eta};
			vector< double > pT_eta11 = {genMu11_pt, genA1Mu1_eta};
			vector< vector< double > > genMus;

			genMus.push_back(pT_eta00);
			genMus.push_back(pT_eta01);
			genMus.push_back(pT_eta10);
			genMus.push_back(pT_eta11);

			int gMu17 = 0;
			int gMu8 = 0;

			for(int i = 0; i < genMus.size(); i++){
				//cout << "genMus pT, eta: " << genMus[i][0] << ", " << genMus[i][1] << endl;
				if(genMus[i][0] > 17 && fabs(genMus[i][1]) < 0.9) gMu17++;
				if(genMus[i][0] > 8  && fabs(genMus[i][1]) < 2.4) gMu8++;
			}
			bool runGen = false;
			if(gMu17 >= 1){
				c1genm++;
										...
										if( (diMuonC_m1_FittedVtx_hitpix_l3inc==1||diMuonC_m2_FittedVtx_hitpix_l3inc==1)&&(diMuonF_m1_FittedVtx_hitpix_l3inc==1||diMuonF_m2_FittedVtx_hitpix_l3inc==1) ){
											...
											if(diMuons_dz_FittedVtx <= 0.1 && diMuons_dz_FittedVtx >= -10){
												...
												if(is2DiMuonsMassOK_FittedVtx){
													...
													brokenCounter++;
													if(diMuonF_IsoTk_FittedVtx <= 2 && diMuonF_IsoTk_FittedVtx >= 0){
														if(diMuonC_IsoTk_FittedVtx <= 2 && diMuonC_IsoTk_FittedVtx >= 0){
	...
	std::cout <<"Nrec/Ntot:                     " << ev_isDiMuonHLTFired/(ev_all*1.0) << endl;
...
void FakeInvestigation()
{
	...
	readTextFileWithSamples("/fdata/hepx/store/user/bmichlin/NMSSM_PATANA_Location.txt", DarkSUSY_mH_125_mGammaD_v);
